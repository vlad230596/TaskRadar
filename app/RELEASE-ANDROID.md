...
## Текущее состояние (16.09.2026): ключ заведён

Всё из инструкции ниже уже выполнено на машине владельца — по его просьбе,
агентом, а не вручную:

| | |
|---|---|
| Keystore | `%USERPROFILE%\keys\taskradar-release.jks` (JKS, RSA-4096, до 01.02.2054) |
| Алиас | `taskradar` |
| Отпечаток SHA-256 | `B9:96:F5:36:98:F0:20:03:F5:93:7A:CB:45:2F:BD:A8:F8:61:F2:C1:3B:C9:93:DA:29:07:E1:1D:BC:82:E2:D2` |
| Пароль | `CREDENTIALS.local.md` в корне репозитория (gitignored) |
| Actions-секреты | все четыре заведены |
| Переменная `TASKRADAR_API_URL` | `https://task-radar.duckdns.org:8443` |
| Локальная сборка | `app/android/key.properties` создан |

**Что остаётся владельцу — бэкап.** Сейчас ключ существует в одном экземпляре, на
одном диске, плюс копия в Actions-секрете, откуда его не прочитать обратно.
Скопируйте `.jks` и пароль туда, где они переживут смерть диска и переустановку
системы: причины — в разделе «Сначала — про потерю ключа». Пока приложение не
установлено ни на одном телефоне, потеря ключа стоит ровно ничего (генерируется
новый); после первой установки — уже нельзя ни потерять, ни заменить.

Отпечаток выше — то, с чем сверяется `apksigner verify --print-certs` на готовом
APK: так проверяется, что скачанный из Release файл подписан именно этим ключом.
...
1. **Ключ — долгоживущий секрет, а не артефакт сборки.** Он создаётся один раз
   на машине владельца и никогда — в CI: сборка получает его копию из
   Actions-секрета, но сгенерировать себе ключ сама не может, иначе каждая
   сборка подписывалась бы новым.
   (Исходно здесь было «создаёт владелец, не агент»; фактически ключ по просьбе
   владельца сгенерировал агент — см. «Текущее состояние». Смысл правила от
   этого не меняется: бэкап и ответственность за ключ остаются на владельце.)
